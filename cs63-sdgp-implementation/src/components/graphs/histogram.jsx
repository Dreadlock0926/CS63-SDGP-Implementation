...
const Marker = ({ position, axis, scale }) => {
  ...
  return <div className='marker' ref={markerRef}>
...
const Axis = ({ numMarkers, size, dim, updateGraphSize, scale }) => {
  ...
  useEffect(() => {
    ...
    updateGraphSize(dim === 'x' ? size : undefined, dim === 'y' ? size : undefined);
  ...
  if (dim === 'x') {
    return (
      <div className='x-axis' ref={AxisRef}>
        {Array.from({ length: numMarkers }).map((_, index) => (
          <Marker key={index} position={(index + 1) * ((size-40) / numMarkers)} axis={'x'} scale={(index+1) * scale}/>
        ))}
      </div>
    );
  } else {
    return (
      <div className='y-axis' ref={AxisRef}>
        {Array.from({ length: numMarkers }).map((_, index) => (
          <Marker key={index} position={(index + 1) * ((size-40) / numMarkers)} axis={'y'} scale={(index+1) * scale}/>
        ))}
      </div>
    );
  }
...
const Histogram = ({ width, height, numXMarkers, numYMarkers, scaleX, scaleY, maxBoxes }) => {
  ...
  const addBox = (boxWidth, boxHeight) => {
    ...
    setAnswerData((prevAnswerData) => [...prevAnswerData, [parseFloat(boxWidth), parseFloat(boxHeight)]]);
    ...
    if (lastPosition.length === 0) {
      ...
      setLastPosition((prevLastPosition) => [...prevLastPosition, parseFloat(boxWidth)]);
   
    } else {
      const newPosition = parseFloat(lastPosition[lastPosition.length - 1]) + parseFloat(boxWidth);
      setLastPosition((prevLastPosition) => [...prevLastPosition, newPosition]);
    }

    const newComponent = <Box key={addedComponents.length} width={boxWidth} height={boxHeight} position={lastPosition[lastPosition.length - 1]} />;
  ...
  return (
    <div className="graph-container">
        <div className='graph' ref={graphRef}>
          <Axis numMarkers={numXMarkers} dim={'x'} size={graphWidth} updateGraphSize={updateGraphSize} scale={scaleX} />
          <Axis numMarkers={numYMarkers} dim={'y'} size={graphHeight} updateGraphSize={updateGraphSize} scale={scaleY} />
    ...
    <form>
        <label>
            Enter width of box:
            <input type="text" name="width" value={enteredBoxWidth} onChange={(e) => setEnteredBoxWidth(e.target.value)} />
        </label>
        <label>
            Enter height of box:
            <input type="text" name="height" value={enteredBoxHeight} onChange={(e) => setEnteredBoxHeight(e.target.value)} />
        </label>
    </form>
    <button onClick={() => addBox(enteredBoxWidth, enteredBoxHeight)} className='add-box'>Add Box</button>
    <button onClick={() => deleteBox()} className='add-box'>Delete Box</button>
    </div>
  );
};
