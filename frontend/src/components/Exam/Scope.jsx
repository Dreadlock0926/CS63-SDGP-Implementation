...
const Scope = () => {
  ...
  const getTopicsFromSource = async (e) => {
    try {
      setLoading(true);
  ...
  const selectScope = async (e) => {
    ...
    try {
      ...
      const exam = await Axios.get("http://localhost:8000/exam/scope", {
    ...
    <div style={{ margin: "5%", padding: "5%" }}>
      {loading ? (
        ...
        <div>
          ...
          <br></br>
          <button onClick={selectAllTopics} disabled={toggle}>
            Select From All
          </button>
          <p>
          ...
          <br></br>
          <button onClick={selectTopics} className="action-button">
            {toggle ? "Close Selector" : `Select Specific Topics!`}
          </button>
          <br></br>
          {toggle ? (
